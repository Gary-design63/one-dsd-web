...
export default async function MeasurementPage({searchParams}:{searchParams?:Promise<WorkOriginInput>}){
 ...
 return <EditableSurfaceRegion surface={surface}><PageIntro title={measurementText(surface.values,'title')} lede={measurementText(surface.values,'intro')}/><div className="wrap max-w-4xl space-y-8 py-8"><WorkOriginLinks origin={origin}/><details className="border-b border-line pb-3 print:hidden"><summary className="min-h-11 cursor-pointer py-2 font-semibold text-[#183247]">Download this evaluation plan</summary><ResourceDownloads kind="measurement" id="worksheet" noun="evaluation plan" scope={scope}/></details><p><Link href={href}>{measurementText(surface.values,'sourceLabel')}: {source.title}</Link></p><MeasurementWorksheet copy={surface.values} source={{title:source.title,href}}/><p className="text-sm text-muted">{TRAINING_CREDIT_NOTICE}</p></div></EditableSurfaceRegion>;
}
