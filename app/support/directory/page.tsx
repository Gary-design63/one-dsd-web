...
export default function SupportDirectoryPage() {
  return <>
    ...
    <div className="wrap max-w-5xl space-y-8 py-8">
      ...
      <details className="border-t border-line pt-4"><summary className="cursor-pointer font-semibold text-[#123f60]">Download this directory</summary><ResourceDownloads kind="support-directory" id="dhs" noun="directory" /></details>
      <div className="space-y-6">{SUPPORT_SOURCES.map(source => <section key={source.id} id={source.id} className="border-t border-line pt-5">
        ...
        <details className="text-sm text-muted"><summary>About this source</summary>
          <p>Published by {source.publisher}.{source.sourceDate ? ` The source identifies its date as ${source.sourceDate}.` : " No publication date is stated here."}</p>
        </details>
      </section>)}</div>
    </div>
  </>;
