...
export default async function CoursePage({params}:{params:Promise<{courseId:string}>}) {
  ...
  return <div className={`course-page ${styles.page}`}><Link className={styles.breadcrumb} href="/learn">← Learning and resources</Link><EditableSurfaceRegion surface={surface}>
    <header className={styles.hero}><div><p className={styles.eyebrow}>{course.seriesLabel}</p><h1>{course.title}</h1><p className={styles.description}>{courseSummary(pack)}</p><p className={styles.duration}>{course.duration} · {course.lessons.length} lessons</p><CourseResume courseId={course.id} lessons={course.lessons.map(({id,title})=>({id,title}))}/><ResourceShare title={course.title} href={`/share/courses/${encodeURIComponent(course.id)}`} noun="course" /></div><div className={styles.cover}><Image src={course.coverImage} alt={course.coverAlt} fill sizes="(max-width: 700px) 90vw, 480px" style={{objectFit:"contain"}} unoptimized /></div></header>
    <nav className={styles.sectionNav} aria-label="In this course"><Link href="#lessons">Lessons</Link><Link href="#job-aid">{pack.jobAid.title}</Link><Link href="#sources">Sources and further reading</Link></nav>
    ...
    {course.learning?<section className={styles.outcomes}><h2>What you’ll be able to do</h2><div><h3>Explore and practice</h3><ul>{course.learning.objectives.map((item,i)=><li key={i}>{item}</li>)}</ul></div><div><h3>What you can take into your work</h3><ul>{course.learning.evidence.map((item,i)=><li key={i}>{item}</li>)}</ul></div><p>{course.learning.appliedNextStep}</p></section>:null}
    <section id="lessons"><h2>Explore the lessons</h2><ol className={styles.lessons}>{course.lessons.map((lesson,i)=><li key={lesson.id}><span className={styles.lessonNumber} aria-hidden="true">{String(i+1).padStart(2,"0")}</span><div><h3><Link href={courseHref(course.id,lesson.id)}>{lesson.title}</Link></h3><p>{lesson.summary}</p></div><span className={styles.lessonTime}>{lesson.minutes} minutes</span></li>)}</ol></section>
