...
export default async function Page() {
  ...
  return <EditableSurfaceRegion surface={{...surface, canEdit: surface.canEdit && editableSurfaceEditingAvailable(process.env, surface.definition.surfaceId)}}><div className={`${styles.page} ${styles.content} ${styles.readingPage} space-y-10`}>
    <header className="max-w-3xl space-y-4"><Link href="/learn/equity-toolkit">{text('back')}</Link><h1 className="text-4xl font-semibold">{text('title')}</h1><p className="text-xl">{text('intro')}</p><p>{text('sourceNote')}</p>{surface.available ? <details className="border-b border-line pb-3 print:hidden"><summary className="min-h-11 cursor-pointer py-2 font-semibold text-[#183247]">Download this page</summary><ResourceDownloads kind="community-connections" id="home" noun="page" /></details> : null}</header>
