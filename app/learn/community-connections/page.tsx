...
export default async function Page() {
  ...
  return <EditableSurfaceRegion surface={{...surface, canEdit: surface.canEdit && editableSurfaceEditingAvailable(process.env, surface.definition.surfaceId)}}><div className={`${styles.page} ${styles.content} ${styles.readingPage} space-y-10`}>
    ...
    <section className="space-y-4"><h2 className="text-2xl font-semibold">{text('planningTitle')}</h2><p>{text('planningIntro')}</p><ol className="list-decimal space-y-4 pl-6">{stringListValue(surface.values,'questions').map(q=><li key={q}>{q}</li>)}</ol></section>
