...
export default async function Page() {
  ...
  return <div className="wrap max-w-6xl space-y-8 py-10">
    ...
    <ul className="grid gap-5 md:grid-cols-2">{ORGANIZATIONAL_AREAS.map((area, i) => <li key={area.id} className="border-t border-line pt-4"><Link href={`/consultant/workforce/${area.id}`}>{surfaces[i].available ? stringValue(surfaces[i].values, "name") : area.name}</Link><p className="text-sm">{surfaces[i].available ? workforceStatusText(stringValue(surfaces[i].values, "status")) : "No released work map yet."}</p></li>)}</ul>
