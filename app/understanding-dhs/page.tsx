...
export default function UnderstandingDhsPage() {
  return <><PageIntro kicker="People, Access and Culture" title="Understanding DHS" lede="People, programs and partnerships across Minnesota DHS, with a closer look at Disability Services and the work we share." />
    <div className="wrap max-w-6xl py-10">
      ...
      <details className="my-7 border-y border-line py-4"><summary className="cursor-pointer font-semibold">Download this DHS reference</summary><div className="mt-4"><ResourceDownloads kind="understanding-dhs" id="reference" noun="reference" compact /></div></details>
      ...
      <div className="space-y-14">{DHS_REFERENCE_GROUPS.map(group=><section key={group.id} id={group.id} aria-labelledby={group.id+"-title"} className="scroll-mt-28">
        <h2 id={group.id+"-title"} className="mb-6 text-3xl font-bold">{group.title}</h2>
        <div className="grid items-start gap-6 md:grid-cols-2">{group.entries.map(entry=><article key={entry.id} id={dhsTopicAnchor(entry.id)} aria-labelledby={dhsTopicAnchor(entry.id)+"-title"} className="scroll-mt-28 rounded-xl border border-line bg-white p-6">
          <h3 id={dhsTopicAnchor(entry.id)+"-title"} className="m-0 text-xl font-bold">{entry.title}</h3>
          <p className="leading-relaxed">{entry.facts}</p>
          <p className="text-sm"><strong>Related teams and partners:</strong> {entry.units.join("; ")}</p>
          <EngagementReferenceMap topicId={entry.id} application={entry.application} />
          <details className="mt-4 border-t border-line pt-3 text-sm">
            <summary className="cursor-pointer font-semibold">Original sources</summary>
            <ul className="mb-0 space-y-2 pl-5">{entry.sourceIds.map(id=>{const source=reference.sources.find(s=>s.id===id)!;return <li key={id}><a href={source.url}>{source.title}</a></li>;})}</ul>
          </details>
        </article>)}</div>
      </section>)}</div>
      <footer className="mt-12 border-t border-line pt-6"><p className="text-sm text-muted">Program summaries accompany the original sources, which carry current policy and contact information.</p><p><Link href="/ask">Browse common questions</Link> <span aria-hidden="true"> · </span> <Link href="/learn">Learning and resources</Link></p></footer>
    </div></>;
}
