...
const DIVERSITY_COURSES = AUTHORED_COURSES.filter(pack => /^div-[fia]/.test(pack.course.id));
...
afterAll(()=>writeEvidenceReceipt("evidence/next-pass-2026-09-08/diversity-course-interactions.json",{environment:"React DOM in jsdom; complete source instances, separate real-browser checks",lessons:receipts.length,blocks:receipts.reduce((sum,row)=>sum+row.blocks,0),receipts}));

for(const pack of DIVERSITY_COURSES) it(`all lesson interactions and saved notes: ${pack.course.id}`,async()=>{
...
it("does not keep a course notes field on staff lessons",async()=>{
  const pack=DIVERSITY_COURSES[0],lesson=sanitizedLesson(pack.course.lessons[0]);const view=render(<CourseLesson courseId={pack.course.id} lesson={lesson} objectives={lessonObjectives(lesson)}/>);await act(async()=>{await Promise.resolve();});
  expect(view.queryByRole("textbox",{name:"Notes to take with you"})).toBeNull();
