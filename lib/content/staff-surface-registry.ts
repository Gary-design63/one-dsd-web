...
const NON_BRIEF_SURFACES: readonly StaffSurfaceDefinition[] = [
  defineSurface({
    ...
    approvedValues: {
      ...
      statesTitle: "What the check labels mean",
      statesBody: "Reached means the address answered when the program checked it on the date shown. Reached, now at a new address means it answered and forwarded to a newer page. Not reached means the address did not answer on that date and may have moved. Not yet checked means the source was cited when the resource was written and the program has not checked the address yet. Checked on a date means the Understanding DHS reference recorded its own check. Reaching an address confirms it exists, not that its content has been reviewed against the resource.",
      usedInLabel: "Used in",
  ...
  defineSurface({
    ...
    approvedValues: {
      ...
      sourcesBody: "Each item shows its source, who maintains it, its review date, what it covers, and its limits. Official sources, program guidance, outside evidence, and material still under review are kept visibly distinct. A proprietary assessment or toolkit may inform professional practice without being copied, scored, or presented as department-owned.",
