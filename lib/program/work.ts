...
async function reviewPublishedResources(task:ProgramWorkView) {
  ...
  const inventory=items.slice(0,70).map(item=>"- "+item.title+" ["+item.id+"]; "+item.type+"; "+item.authority+"; review "+(item.reviewDate||"not recorded")+".");
  const body=["Published-resource review for "+task.title,"Checked: "+new Date().toISOString(),"Sources: "+snapshots.map(s=>s.requestedScope+" / "+s.source).join(", "),items.length+" published resources matched this area using its recorded title, summary and tags.",...inventory,items.length>70?"The first 70 matches are listed; the counts include all matches.":"","Findings",...(findings.length?findings.slice(0,40):["No missing reading content, pending accessibility status or past review date was found in the selected records."]),items.length===0?"No matching published resources were found. This gap needs follow-through.":"","Next action: use the linked resources to review the findings and assign any content repair. Review dates and recorded accessibility labels are metadata; this check does not recertify source facts, conduct manual accessibility testing, or establish staff application or benefit.",TRAINING_CREDIT_NOTICE].filter(Boolean).join("\n\n").slice(0,17500);
...
async function executeTask(task:ProgramWorkView):Promise<NonNullable<ProgramEvent["receipt"]>> {
  ...
  if(task.kind==="resource_review"){
    ...
    body=`Source access checked ${new Date().toISOString()}. HTTP ${response.status}. Retrieved content SHA-256: ${hash(content)}. The source was accessible. This receipt is an access check, not a factual recertification; the canonical source review date remains unchanged.`;
